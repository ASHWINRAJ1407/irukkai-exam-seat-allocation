...
class Subject(db.Model):
    ...
    Dual-Attribute Integrity: Unique Constraint on (name, code) per user.
    This allows multiple entries for "Deep Learning" if codes differ (e.g. AIDS vs AIML),
    but the application layer treats all rows sharing the same subject_name as one logical group.
    """
    ...
    __table_args__ = (
        db.UniqueConstraint('owner_user_id', 'name', 'code', name='unique_subject_name_code_per_user'),
    )
