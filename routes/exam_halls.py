...
def index():
    ...
    query = ExamHall.query.order_by(ExamHall.hall_number)
    query = query.filter(ExamHall.owner_user_id == current_user.id)
    halls = query.all()
...
def delete(id):
    hall = ExamHall.query.get_or_404(id)
    if not current_user.is_admin and hall.owner_user_id != current_user.id:
