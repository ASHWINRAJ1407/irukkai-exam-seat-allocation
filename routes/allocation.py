...
from utils.result_automation.automation import generate_report_flask
...
@allocation_bp.route('/mark-analysis/<int:exam_id>/<exam_date>', methods=['GET', 'POST'])
@login_required
def mark_analysis(exam_id, exam_date):
    """Integrate Result Automation feature (mark analysis) into allocation UI."""
    ...
    if request.method == 'POST':
        ...
        college_name = request.form.get('college_name') or ''
        uploaded_files = request.files.getlist('files')

        if not uploaded_files or uploaded_files[0].filename == '':
            flash('Please upload at least one PDF file.', 'warning')
            return redirect(url_for('allocation.mark_analysis', exam_id=exam_id, exam_date=exam_date))

        # Prepare folders inside the main app uploads directory and isolate per-user
        base_upload = current_app.config.get('UPLOAD_FOLDER')
        if not isinstance(base_upload, str):
            base_upload = str(base_upload)

        user_id = getattr(current_user, 'id', 'anonymous')
        ra_input = os.path.join(base_upload, 'result_automation', 'input', str(user_id))
        ra_output = os.path.join(base_upload, 'result_automation', 'output', str(user_id))
        os.makedirs(ra_input, exist_ok=True)
        os.makedirs(ra_output, exist_ok=True)

        # Clear previous input for this user only
        for f in os.listdir(ra_input):
            try:
                os.remove(os.path.join(ra_input, f))
            except Exception:
                pass

        # Save uploaded PDFs to the user's input folder
        for f in uploaded_files:
            filename = secure_filename(f.filename)
            f.save(os.path.join(ra_input, filename))

        try:
            output_path = generate_report_flask(exam_type, college_name, ra_input, ra_output)
        except Exception:
            flash('An error occurred while processing the uploaded PDFs.', 'danger')
            return redirect(url_for('allocation.mark_analysis', exam_id=exam_id, exam_date=exam_date))

        if not output_path or not os.path.exists(output_path):
            flash('No report was generated. Ensure the PDFs contain valid mark data.', 'warning')
            return redirect(url_for('allocation.mark_analysis', exam_id=exam_id, exam_date=exam_date))

        # Prepare list of generated files (user-specific)
        files = sorted(os.listdir(ra_output)) if os.path.exists(ra_output) else []
        # Record usage event for mark analysis
        try:
            record_usage(current_user, "mark_analysis")
        except Exception:
            # Don't break generation on logging failure
            pass
        if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
            return jsonify({'success': True, 'files': files, 'message': 'Report generated successfully.'})
        flash('Report generated successfully.', 'success')
        return redirect(url_for('allocation.mark_analysis_home'))

    return render_template('allocation/mark_analysis.html', exam_id=exam_id, exam_date=exam_date, exam_name=None)


@allocation_bp.route('/mark-analysis', methods=['GET', 'POST'])
@login_required
def mark_analysis_home():
    """Generic mark analysis page not tied to a specific exam/date."""
    if request.method == 'POST':
        exam_type = request.form.get('exam_type')
        college_name = request.form.get('college_name') or ''
        uploaded_files = request.files.getlist('files')
    ...
    # On GET, list generated reports for the current user
    base_upload = current_app.config.get('UPLOAD_FOLDER')
    if not isinstance(base_upload, str):
        base_upload = str(base_upload)
    user_id = getattr(current_user, 'id', 'anonymous')
    ra_output = os.path.join(base_upload, 'result_automation', 'output', str(user_id))
    generated = sorted(os.listdir(ra_output)) if os.path.exists(ra_output) else []
    return render_template('allocation/mark_analysis.html', exam_id=None, exam_date=None, exam_name=None, generated_reports=generated)


@allocation_bp.route('/download-result/<path:filename>')
@login_required
def download_result(filename):
    # Serve files only from the current user's output folder
    base_upload = current_app.config.get('UPLOAD_FOLDER')
    if not isinstance(base_upload, str):
        base_upload = str(base_upload)
    user_id = getattr(current_user, 'id', 'anonymous')
    ...
    file_path = os.path.join(ra_output, safe_name)
...
@allocation_bp.route('/mark-analysis/get-reports', methods=['GET'])
@login_required
def get_generated_reports():
    """Return list of generated reports for the current user as JSON."""
    base_upload = current_app.config.get('UPLOAD_FOLDER')
    if not isinstance(base_upload, str):
        base_upload = str(base_upload)
    user_id = getattr(current_user, 'id', 'anonymous')
...
@allocation_bp.route('/mark-analysis/delete/<path:filename>', methods=['POST'])
@login_required
def mark_analysis_delete(filename):
    # Delete a generated report for the current user
    base_upload = current_app.config.get('UPLOAD_FOLDER')
    if not isinstance(base_upload, str):
        base_upload = str(base_upload)
    user_id = getattr(current_user, 'id', 'anonymous')
