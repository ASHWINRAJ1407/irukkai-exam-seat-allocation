...
def generate_form():
    ...
    if not start_date_str:
        ...
        return redirect(url_for('timetable_generator.generate_form'))
    if not end_date_str:
        flash('End date is required. Please select the last date for allocation.', 'danger')
        return redirect(url_for('timetable_generator.generate_form'))
    ...
    try:
        # Pass subject_names_map and end_date to generate_timetable for subject-name-based conflict resolution
        # generate_timetable now returns (schedule, subject_names_map, warning)
        # We pass end_date via the subject_names_map special key '__end_date__'
        sn_map = dict(subject_names_map) if subject_names_map else {}
        sn_map['__end_date__'] = end_date_str
        schedule_res = generate_timetable(rows, start_date_str, excluded, subject_names_map=sn_map)
        # Accept both old (2-tuple) and new (3-tuple) signatures
        if isinstance(schedule_res, tuple) and len(schedule_res) == 3:
            schedule, subject_names_map, gen_warning = schedule_res
        elif isinstance(schedule_res, tuple) and len(schedule_res) == 2:
            schedule, subject_names_map = schedule_res
            gen_warning = None
        else:
            raise ValueError('Unexpected result from timetable generator')
        if gen_warning:
            flash(gen_warning, 'warning')
    except ValueError as e:
        flash(str(e), 'danger')
        return redirect(url_for('timetable_generator.generate_form'))
    ...
    # No subscription enforcement: permit timetable generation for authenticated users.
...
def make_scheduling():
    ...
    # No subscription enforcement: permit scheduling for authenticated users.
