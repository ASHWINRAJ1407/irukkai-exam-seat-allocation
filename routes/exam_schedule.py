...
@login_required
def index():
    query = Exam.query.order_by(Exam.id.desc())
    if not current_user.is_admin:
        query = query.filter(Exam.owner_user_id == current_user.id)
    exams = query.all()
...
@login_required
def add():
    dept_q = Department.query.order_by(Department.code)
    subj_q = Subject.query.order_by(Subject.code)
    if not current_user.is_admin:
...
def detail(exam_id):
    ...
    sched_q = ExamSchedule.query.filter_by(exam_id=exam_id).order_by(
...
def delete_all():
    ...
    if exam_id:
        ...
        if exam and (current_user.is_admin or exam.owner_user_id == current_user.id):
            SeatAllocation.query.filter_by(exam_id=exam_id).delete()
            ExamSchedule.query.filter_by(exam_id=exam_id).delete()
...
def delete_entry(id):
    ...
    if exam and (not current_user.is_admin and exam.owner_user_id != current_user.id):
