...
def index():
    ...
    # Always scope student queries to the logged-in user
    query = Student.query.filter(Student.owner_user_id == current_user.id)
    if dept_id:
    ...
    dept_query = Department.query.order_by(Department.code).filter(Department.owner_user_id == current_user.id)
    ...
    sections = db.session.query(Student.section).distinct().filter(Student.owner_user_id == current_user.id, Student.section.isnot(None), Student.section != '').all()
...
def add():
    dept_query = Department.query.order_by(Department.code).filter(Department.owner_user_id == current_user.id)
    departments = dept_query.all()
    batch_query = db.session.query(Student.academic_year).filter(Student.owner_user_id == current_user.id)
    ...
    if request.method == 'POST':
        ...
        try:
            db.session.add(s)
            # Update department totals only for departments owned by this user
            try:
                dept.total_students = Student.query.filter_by(department_id=dept_id, owner_user_id=current_user.id).count() + 1
            except Exception:
...
def import_students():
    ...
    if request.method == 'POST':
        ...
        dept.total_students = Student.query.filter_by(department_id=dept_id, owner_user_id=current_user.id).count()
        db.session.commit()
...
def delete_filtered():
    ...
    for dept in Department.query.filter(Department.owner_user_id == current_user.id).all():
        dept.total_students = Student.query.filter_by(department_id=dept.id, owner_user_id=current_user.id).count()
    db.session.commit()
...
@login_required
def delete_all():
    # Only allow deleting current user's students; admin account cannot operate on tenant data here
    if current_user.is_admin:
        flash('Admin account is not allowed to delete tenant student data from this page.', 'danger')
        return redirect(url_for('students.index'))
    base_q = Student.query.filter(Student.owner_user_id == current_user.id)
    count = base_q.count()
    if count == 0:
